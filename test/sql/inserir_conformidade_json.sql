...
DECLARE
    ...
    v_antes BIGINT;
BEGIN
    INSERT INTO workspace (nome, cnpj) VALUES ('Teste SCRUM-174', '99999999999174') RETURNING id_workspace INTO v_workspace;
    ...
    INSERT INTO nr_catalogo (codigo_nr, titulo, tempo_reciclagem_mes, revogada)
    ...
    CALL inserir_conformidade_json(v_payload::TEXT, v_erros);
    ...
    ASSERT (SELECT COUNT(*) = 2 FROM conformidade WHERE usuario_id = v_usuario);
    ASSERT (SELECT BOOL_AND(aplicavel AND origem = 'MANUAL' AND conclusao_evento_id IS NULL)
            FROM conformidade WHERE usuario_id = v_usuario), 'Defaults da conformidade manual';
    ASSERT (SELECT nome = 'Funcionário teste' FROM usuario WHERE id_usuario = v_usuario), 'Não altera o cadastro';

    -- Atualização não duplica; a última entrada válida do mesmo par vence.
    CALL inserir_conformidade_json('[
    ...
    ASSERT (SELECT COUNT(*) = 2 FROM conformidade WHERE usuario_id = v_usuario);
    ASSERT (SELECT data_validade = DATE '2029-03-01' FROM conformidade WHERE usuario_id = v_usuario AND nr_id = 99174);
    ASSERT (SELECT data_validade = DATE '2027-03-10' FROM conformidade WHERE usuario_id = v_usuario AND nr_id = 99175);

    -- Preserva origem, aplicabilidade e vínculo com a conclusão nas linhas existentes.
    INSERT INTO evento (gestor_id, nr_id, titulo, descricao, modo_conclusao)
    VALUES (v_gestor, 99174, 'Evento teste', 'Descrição teste', 'GESTOR') RETURNING id_evento INTO v_evento;
    INSERT INTO turma (evento_id, nome, data_inicial, data_termino)
    VALUES (v_evento, 'Turma teste', '2026-01-01 08:00:00', '2026-01-01 12:00:00') RETURNING id_turma INTO v_turma;
    INSERT INTO turma_funcionario (turma_id, usuario_id) VALUES (v_turma, v_usuario) RETURNING id_turma_funcionario INTO v_participacao;
    INSERT INTO conclusao_evento (turma_funcionario_id, status, data_conclusao, data_validacao, data_validade)
    ...
    INSERT INTO conformidade (usuario_id, nr_id, aplicavel, origem, conclusao_evento_id, data_validade)
    VALUES (v_usuario, 99174, FALSE, 'CONCLUSAO_EVENTO', v_conclusao, '2027-01-01') RETURNING id_conformidade INTO v_conformidade;
    CALL inserir_conformidade_json('[{"email":"scrum174@example.com","nr":99174,"dataValidade":"2030-01-01"}]', v_erros);
    ASSERT v_erros = '[]'::JSONB;
    ASSERT (SELECT COUNT(*) = 2 AND BOOL_AND(data_validade = DATE '2030-01-01') FROM conformidade WHERE usuario_id = v_usuario AND nr_id = 99174);
    ASSERT (SELECT NOT aplicavel AND origem = 'CONCLUSAO_EVENTO' AND conclusao_evento_id = v_conclusao FROM conformidade WHERE id_conformidade = v_conformidade);
    ASSERT (SELECT data_validade = DATE '2027-01-01' FROM conclusao_evento WHERE id_conclusao_evento = v_conclusao);

    -- Erros de formato e referências não escrevem registros, nem interrompem o lote.
    SELECT COUNT(*) INTO v_antes FROM conformidade;
    CALL inserir_conformidade_json('[
        null,
        {"email":""},
    ...
    ASSERT (SELECT BOOL_AND(data_validade = DATE '2030-01-01') FROM conformidade WHERE usuario_id = v_usuario AND nr_id = 99174);

    CALL inserir_conformidade_json('não é json', v_erros);
    ASSERT v_erros -> 0 ->> 'erro' = 'JSON inválido.';
    CALL inserir_conformidade_json('{}', v_erros);
    ASSERT v_erros -> 0 ->> 'erro' = 'O JSON deve ser um array de objetos.';
    CALL inserir_conformidade_json(NULL, v_erros);
