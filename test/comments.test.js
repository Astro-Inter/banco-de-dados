...
test('o snapshot gerado leva as descrições para a interface', async () => {
  ...
  const workspace = database.objects.find((object) => object.name === 'workspace');
  assert.match(workspace.description, /ambientes clientes/);
  assert.equal(workspace.columns.find((column) => column.name === 'nome').description, 'Nome empresarial ou nome de exibição do workspace.');
  ...
  assert.match(script.content, /CREATE TABLE workspace/);
  assert.match(script.content, /COMMENT ON TABLE workspace/);
});
