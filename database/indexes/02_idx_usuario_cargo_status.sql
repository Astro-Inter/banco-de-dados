...
FROM usuario
WHERE cargo_id = (SELECT MIN(id_cargo) FROM cargo)
  AND status = 'ATIVO'
ORDER BY nome;

CREATE INDEX IF NOT EXISTS idx_usuario_cargo_status
    ON usuario (cargo_id, status);

COMMENT ON INDEX idx_usuario_cargo_status IS
