...
SELECT cargo_id
FROM cargo_nr
WHERE nr_id = (SELECT MIN(nr_id) FROM cargo_nr)
ORDER BY cargo_id;

CREATE INDEX IF NOT EXISTS idx_cargo_nr_nr_cargo
    ON cargo_nr (nr_id, cargo_id);

COMMENT ON INDEX idx_cargo_nr_nr_cargo IS
